...
processData <- function(Y.Name = "NumBoc", common_name = "Bocaccio", Grand = Grand_2019, Include.FishTime = FALSE, Restrict.6min = FALSE, Sites = NULL, Area = c("Orig121", "CCA", "ALL")[1], 
   ...
   names(list.tmp) <- paste("mean", Y.Name, sep=".")
   catf("\n"); printf(r(cbind(aggregate(list.tmp, list(CrewStaff = Grand.TMP$CrewStaff, vessel = Grand.TMP$vessel), mean, na.rm = TRUE), 
                      N = aggregate(list.tmp, list(Grand.TMP$CrewStaff, vessel = Grand.TMP$vessel), length)[,2])[,c("vessel", "CrewStaff", "N", "mean.NumBoc")]), 2); catf("\n\n")
